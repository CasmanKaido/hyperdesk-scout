...
5. Evidence generation, grounded AI analysis failure/unavailability, or pre-settlement storage failure abandons the operation without calling settlement. Only a complete artifact with `analysis.status: completed` can reach facilitator settlement (`POST /settle`). Immediate confirmed success releases the report with a base64 `PAYMENT-RESPONSE` receipt and `report.payment` metadata.
...
- **Validate and build before charge.** Request input is validated and rate-limited before verification; after verification, the full report is generated and persisted before settlement. Evidence failure or unavailable grounded AI synthesis never invokes `/settle`; the buyer receives retryable `503 report_generation_unavailable` instead.
...
Production v0.8.7 completed two fresh official buyer flows for BTC reports at 0.01 USD₮0 (10,000 atomic units) each on X Layer testnet. Transactions `0x07f6fc456ac7f05e62eecb1c7330aa028229133d25efcfce2658a0cf7b840715` and `0xac3d8a1cae087dd398b62a4bac34930eb1c352501543e5979529429a5cd45996` were distinct, and both delivered artifacts carried `recovered: true`, proving repeat authorization plus exact-retry reconciliation without another settlement or report generation. The first delivered completed grounded AI analysis. The second settled while AI providers were unavailable, exposing an unacceptable degraded-artifact charge; deployed v0.8.8 now requires completed analysis before settlement. A separate fresh BTC-bound authorization replayed with ETH was rejected as `payment_request_binding_mismatch` with no receipt or transaction hash, proving request-bound access denial before settlement. The payer and recipient were the same wallet, so this is mechanical protocol proof rather than evidence of third-party revenue. Independent-buyer economics, durable storage, and mainnet behavior remain unproven.
...
1. **OKX.AI marketplace** (primary free discovery): Agent ID `13784` was resubmitted with the free Funding Specialist and Market-Neutral Orchestrator after OKX rejected the testnet-paid service; listing review is pending.
2. **Direct paid API**: any compatible client (including `onchainos payment quote`) can probe, confirm, and pay the 0.01 USD₮0 X Layer testnet endpoint. Two direct official payment flows are proven; this endpoint is not in the current marketplace submission.
...
| 4 | Publish research report as a paid A2MCP service on OKX.AI; keep a free tier for discovery | Paid testnet service rejected because marketplace requires X Layer mainnet; removed. Agent resubmitted with two free services; review pending |
