...
Implementation status (2026-09-24): conversational information and strategy journeys are deployed with automated and live-browser coverage. The browser clearly separates free information/first-party analysis from the paid Hyperliquid Research Report, which is published on OKX.AI and enforced by the OKX Agent Payments Protocol. Two direct official testnet payments are proven; marketplace-native payment remains blocked by testnet asset support. The browser does not connect wallets, hold custody, or execute trades. See [`AUDIT.md`](AUDIT.md) for evidence and limitations.
