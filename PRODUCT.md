...
Implementation status (2026-09-24): conversational information and strategy journeys are deployed with automated and live-browser coverage. The browser clearly separates free information/first-party analysis from the direct paid Hyperliquid Research Report, which is enforced by the OKX Agent Payments Protocol. Two direct official testnet payments are proven. The paid marketplace entry was removed after OKX required X Layer mainnet; Agent `13784` was resubmitted with its two free services and is under review. The browser does not connect wallets, hold custody, or execute trades. See [`AUDIT.md`](AUDIT.md) for evidence and limitations.
