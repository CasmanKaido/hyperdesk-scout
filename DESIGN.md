...
- **Information first:** Asset questions and short follow-ups remain information requests. Show a direct conclusion first, followed by cited findings from snapshot, 72-hour funding, and visible order-book records. Keep calculations, source/fetch time, stale or missing evidence, and limitations expandable without strategy constraints or an approval-to-invest implication.
...
- **Results:** Grounded AI interpretation appears first when available; its findings display server-issued evidence IDs. A deterministic market summary remains the fallback. Facts, candidate comparison, workflow trace, and structured evidence are expandable. Previous results are retained as collapsed, read-only snapshots without duplicate DOM IDs.
